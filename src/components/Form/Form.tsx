...
const Form = () => {
  ...
  const { professionalName, day, hour, serviceName, setNameContext, setPhoneContext } = useContext(ScheduleContext);
  ...
  const postSchedule = async (data: DataSchedule) => {
    try {
      const response = await axios.post("http://localhost:3000/schedule/", data);
  ...
  const handleClick = (e: React.FormEvent) => {
    ...
    if (data.client_name === "" || !validatePhoneNumber(formattedPhone)) {
      alert("Dados estão incompletos ou telefone está em formato incorreto. Não é possível agendar.");
      return;
    }
      ...
      <div className={styles.containerDados}>
        <form onSubmit={handleClick}>
          <div className={styles.containerInput}>
            ...
            <input placeholder="Seu nome e sobrenome" id="name" onChange={(e) => setName(e.target.value)} type="text" />
          </div>
          <div className={styles.containerInput}>
          ...
          <input className={styles.buttonShedule} type="submit" value="Agendar" />
        </form>
      </div>
      <Footer />
    </div>
